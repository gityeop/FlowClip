...
class ClipboardTests: XCTestCase {
  ...
  let autoGeneratedType = NSPasteboard.PasteboardType.autoGenerated
  ...
  let sourceType = NSPasteboard.PasteboardType.source
  ...
  @MainActor
  private func captureAutoGeneratedCopy(
    _ text: String,
    source: String,
    ...
    item.setString(text, forType: stringType)
    item.setString(source, forType: sourceType)
    item.setString("", forType: autoGeneratedType)
    additionalTypes.forEach({ item.setString("", forType: $0) })
  ...
  @MainActor
  func testAllowsRaycastBetaAutoGeneratedHexColor() {
    let copiedItem = captureAutoGeneratedCopy("#12AB34", source: "com.raycast-x.macos")

    XCTAssertEqual(copiedItem?.text, "#12AB34")
    XCTAssertFalse(copiedItem?.contents.contains(where: { $0.type == autoGeneratedType.rawValue }) ?? true)
  }

  @MainActor
  func testIgnoresRaycastBetaAutoGeneratedNonColor() {
    XCTAssertNil(captureAutoGeneratedCopy("temporary value", source: "com.raycast-x.macos"))
  }

  @MainActor
  func testIgnoresOtherAppsAutoGeneratedHexColor() {
    XCTAssertNil(captureAutoGeneratedCopy("#12AB34", source: "com.example.app"))
  }

  @MainActor
  func testIgnoresRaycastBetaAutoGeneratedTransientColor() {
    XCTAssertNil(captureAutoGeneratedCopy(
      "#12AB34",
      source: "com.raycast-x.macos",
      additionalTypes: [.transient]
    ))
  }

  @MainActor
  func testIgnoresRaycastBetaAutoGeneratedConcealedColor() {
    XCTAssertNil(captureAutoGeneratedCopy(
      "#12AB34",
      source: "com.raycast-x.macos",
      additionalTypes: [.concealed]
    ))
  }
