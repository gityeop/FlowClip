...
class Clipboard {
  ...
  private let raycastBetaBundleIdentifier = "com.raycast-x.macos"
  ...
  private func shouldIgnore(_ types: Set<NSPasteboard.PasteboardType>) -> Bool {
    ...
    guard !types.isDisjoint(with: enabledTypes) else {
      return true
    }

    let matchedIgnoredTypes = types.intersection(ignoredTypes)
    if matchedIgnoredTypes == [.autoGenerated], isRaycastBetaHexColor {
      return false
    }

    return !matchedIgnoredTypes.isEmpty
  }

  private var isRaycastBetaHexColor: Bool {
    guard pasteboard.string(forType: .source) == raycastBetaBundleIdentifier,
          let string = pasteboard.string(forType: .string) else {
      return false
    }

    return NSColor(hexString: string) != nil
  }
